    ...
    class BillsController < ApplicationController
      ...
      def index
        if params[:enrollment_id]
          ...
          render json: {status: 'SUCCESSO', message:"Faturas da matrícula #{params[:enrollment_id]} carregadas.", data:bills},status: :ok
        elsif params[:student_id]
          bills = Bill.joins(:student).where("student_id = ?", params[:student_id]).order('id ASC');
          render json: {status: 'SUCCESSO', message:"Faturas do estudante #{params[:student_id]} carregadas.", data:bills},status: :ok
        else 
      ...
      def show
        ...
        render json: {status: 'SUCCESSO', message:"Fatura #{params[:id]} Carregada.", data:bill},status: :ok
      end
      ...
      # Atualiza o status de uma fatura
			def update
				...
				if bill.update_attributes(bill_params)
					render json: {status: 'SUCCESSO', message:"Fatura #{params[:id]} Atualizada.", data:bill},status: :ok
				else
      ...
      # Deleta uma fatura
			def destroy
				bill = Bill.find(params[:id])
				bill.destroy
				render json: {status: 'SUCCESSO', message:"Fatura #{params[:id]} Deletada.", data:bill},status: :ok
			end
