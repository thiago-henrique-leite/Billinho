module Api
    ...
    class InstitutionsController < ApplicationController  
        ...
        institution = Institution.find(params[:id])
				render json: {status: 'SUCCESSO', message:"Instituição de Ensino #{params[:id]} Carregada.", data:institution},status: :ok
      end
			...
			def create
				...
				if institution.save
					render json: {status: 'SUCCESSO', message:"#{institution_params[:nome]} Cadastrada.", data:institution},status: :ok
				else
			...
			def update
				...
				if institution.update_attributes(institution_params)
					render json: {status: 'SUCCESSO', message:"Instituição #{params[:id]} Atualizada.", data:institution},status: :ok
				else
			...
			def destroy
				institution = Institution.find(params[:id])
				institution.destroy
				render json: {status: 'SUCCESSO', message:"Instituição #{params[:id]} deletada", data:institution},status: :ok
			end
